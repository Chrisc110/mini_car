...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2)
    {
        ...
        else
        {
            ...
            {
                ultrasonic_config[CHANNEL0].distance = ((0xFFFFFFFF - ultrasonic_config[CHANNEL0].time0 + ultrasonic_config[CHANNEL0].time1) * TIME_DISTANCE_CONVERSION_CM) / 2.0;
            }

            ultrasonic_config[CHANNEL0].calculation_complete = true;
            ultrasonic_config[CHANNEL0].is_time0_captured = false;

            __HAL_TIM_SET_CAPTUREPOLARITY(htim, ultrasonic_config[CHANNEL0].timer_channel, TIM_INPUTCHANNELPOLARITY_RISING);
